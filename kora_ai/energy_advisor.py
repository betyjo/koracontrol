...
Publishes to kora/ai/energy-advisor MQTT topic.
"""
...
class EnergyRecommendation:
    ...
    category: str           # scheduling, efficiency, demand, renewable
...
class EnergyAdvisor:
    ...
    def analyze_scheduling(self) -> Optional[EnergyRecommendation]:
        ...
        if len(self.hourly_consumption) < 6:
            return None
        ...
        peak_consumption = sum(
            sum(self.hourly_consumption[h]) for h in peak_hours
        )
        mid_peak_consumption = sum(
            sum(self.hourly_consumption[h]) for h in mid_peak_hours
        )
        off_peak_consumption = sum(
            sum(self.hourly_consumption[h]) for h in off_peak_hours
        )
        ...
        # If more than 30% of consumption is during peak hours, suggest rescheduling
        if peak_pct > 30:
    ...
    def analyze_efficiency(self) -> Optional[EnergyRecommendation]:
        ...
        if len(self.flow_window) < 20 or len(self.pressure_window) < 20:
            return None
        ...
        throttling_score = 0
        if avg_pressure > optimal_pressure * 1.2 and avg_flow < optimal_flow * 0.8:
        ...
        recommendations = []
        if flow_deviation > 30:
            recommendations.append(f'Average flow ({avg_flow:.0f} L/min) is {flow_deviation:.0f}% from optimal ({optimal_flow} L/min).')
        
        if throttling_score > 10:
        ...
        if par > 2.5:
            ...
            return EnergyRecommendation(
                ...
                description=(
                    ...
                    f'High peak demand increases capacity charges. '
                    f'Stagger pump starts and implement soft-start to reduce peak demand.'
                ),
    ...
    def get_energy_metrics(self) -> EnergyMetrics:
        ...
        # Estimate daily consumption (assuming 1 reading per second)
        ...
        monthly_volume_m3 = np.mean(flows) * 60 * 24 * 30 / 1000 if len(self.flow_window) > 0 else 1
    ...
    def process_sensor_data(self, data: Dict) -> Dict:
        ...
        pump_on = pump_status == 'ON' or data.get('P', 0) == 1
...
def on_connect(client, userdata, flags, reason_code, properties=None):
    print('⚡ Energy Advisor connected to MQTT broker.')
...
def on_message(client, userdata, msg):
    try:
        ...
        result = advisor.process_sensor_data(data)
        
        if result and result.get('recommendations'):
            client.publish('kora/ai/energy-advisor', json.dumps(result))
            savings = result.get('estimated_monthly_savings_etb', 0)
            count = result.get('total_recommendations', 0)
            print(f'⚡ ENERGY: {count} recommendations, est. savings: {savings:.2f} ETB/month')
        
        # Periodically publish metrics
        now = time.time()
        if now - advisor.last_publish < 5:  # Close to publish time
            metrics = advisor.get_energy_metrics()
            client.publish('kora/ai/energy-metrics', json.dumps(asdict(metrics)))
    
    except json.JSONDecodeError as e:
        print(f'❌ JSON decode error in energy_advisor: {e}')
    except Exception as e:
        print(f'❌ Error in energy_advisor: {e}')
...
def main():
    ...
    try:
        print('🔗 Energy Advisor connecting to MQTT broker at localhost:1883...')
        client.connect('localhost', 1883, 60)
        client.loop_forever()
    except Exception as e:
        print(f'❌ Energy Advisor failed to connect: {e}')
